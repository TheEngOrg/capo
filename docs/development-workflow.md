...
Classification happens **at intake** (Step 0), before any agent spawns. This determines the track: MECHANICAL (1 spawn) or ARCHITECTURAL (4-5 spawns).
...
└───────────┬───────────┘               └────────────┬────────────┘
            ...
            ▼                                        ▼
┌───────────────────────┐               ┌─────────────────────────┐
│   Dev (single spawn)  │               │  QA writes tests        │
│   Writes tests +      │               │  (can run parallel      │
│   implements (TDD)    │               │   with Dev)             │
└───────────┬───────────┘               └────────────┬────────────┘
            │                                        │
            ▼                                        ▼
┌───────────────────────┐               ┌─────────────────────────┐
│   BASH GATE           │               │  Dev implements          │
│   Auto-verifies:      │               │  against QA tests        │
│   tests, coverage,    │               └────────────┬────────────┘
│   line limits         │                            │
└───────────┬───────────┘                            ▼
            │                           ┌─────────────────────────┐
            ▼                           │  Dual-specialist review  │
         DONE                          │  (if code blocks present)│
                                       └────────────┬────────────┘
                                                    │
                                                    ▼
                                       ┌─────────────────────────┐
...
Classification happens BEFORE any agent spawns, based on the ticket or request description. The canonical mechanical classification path is `.claude/scripts/teo-classify-workstream`; run it with the workstream ID or pipe a file list via stdin before any agent spawn decision.
...
**User overrides** (on any `/teo-build` invocation):
...
```
Step 1: QA writes tests (can run parallel with Dev)
Step 2: Dev implements against QA tests
Step 2.5: Dual-specialist review (if deliverable contains code blocks)
Step 3: Staff Engineer internal review
Step 4: /deployment-engineer merge
```

### QA (Step 1) — can overlap with Dev
...
### Dual-Specialist Review (Step 2.5 — conditional)
...
**Output:** Approval to merge

### /deployment-engineer (Step 4)
...
| QA | None | Test specs (Step 1) |
| Staff Engineer | None | Review + approval (Step 3) |
| /deployment-engineer | None | Merge (Step 4) |
...
```
/teo-build execute workstream WS-1: [description from leadership plan]
```

### Force a track
```
/teo-build execute WS-1 --force-mechanical
/teo-build execute WS-1 --force-architectural
```

### Merge after staff-engineer approval (ARCHITECTURAL)
```
...
|------|-----|-------|---------|---------|
| User | teo-build | Both | New workstream | `/teo-build execute WS-X` |
| teo-build | dev | MECHANICAL | Classified at intake | Combined test+impl prompt |
| teo-build | qa | ARCHITECTURAL | After classification | Write failing tests |
| qa | teo-build | ARCHITECTURAL | Tests committed | `/teo-build execute WS-X` |
| teo-build | staff-engineer | ARCHITECTURAL | Workstream complete | Review for approval |
| staff-engineer | deployment-engineer | ARCHITECTURAL | Approved | `/deployment-engineer merge` |
| staff-engineer | teo-build | ARCHITECTURAL | Rejected | Specific feedback |
...
> Docs and tests are part of done — address-or-justify. A justified skip (pure internal refactor, no public API change) is valid. An unjustified skip is a BLOCK at this gate.
...
The orchestrator (teo-build) reads all memory files and full source documents, then passes only relevant subsets to task agents:
...
Large deliverable documents (PRD, Technical Design, Brand Kit) are compressed via template-based extraction before being forwarded to task agents. The orchestrator (teo-build) reads the full document; task agents (dev, qa, design) receive only the compressed version — never the raw full document.
...
## Write-Delivery Gate (Wave 2 — Protected Paths)

When a workstream deliverable targets a protected path (`.claude/scripts/**`, `.claude/hooks/**`, `.claude/shared/**`, `docs/**`, `src/**`, `packages/**`), the write-delivery step MUST route through `teo-apply-edit`:

**Gate annotation (MECHANICAL and ARCHITECTURAL):**
- [ ] Any writes to protected paths use `teo-apply-edit` with a valid patch spec (schema_version: "1.0.0")
- [ ] Direct `Edit` or `Write` on protected paths is NOT used (hook will block with exit 2)
- [ ] Audit log entry appears in `.claude/memory/traces/edit-audit-YYYY-MM-DD.json`
