...
| OQ-3 | `bun build --define` behavior with very long PEM strings (>1KB). SPIKE-002 Test 6 confirmed cases up to 97 characters; an actual Ed25519 public key in PEM format is longer. Verify that there is no undocumented length ceiling in Bun's `--define` implementation before M1 release build. SPIKE-002 Test 6 Case B verified 97-character PEM strings. Realistic Ed25519 PEM strings (PKCS8/OpenSSH format) are 120+ characters — base64 body alone is ~68 chars, plus headers, footer, and escaped newlines. The gap between 97 chars tested and actual key length is non-trivial. Staff-Engineer runs a Test 6 variant with a full-length real PEM string before the M1 release build is cut. Required gate, not a blocker for M1 implementation start. | Staff-Engineer | OPEN — required gate before M1 release build |
