...
Loaded at every Capo session start. These define identity, formatting basics, and engineering standards.
...
### Tier 2: Pipeline (Capo composes)

Loaded when the Capo begins pipeline composition (step 6-8 of intake). These are the runtime protocols.
...
2. **Tier 2 protocols are loaded just-in-time** when the Capo reaches the relevant pipeline step. Load only the sections needed for that step.
3. **Tier 3 protocols are never loaded proactively.** The Capo reads them only when the trigger condition fires.
