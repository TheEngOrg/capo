...
**MECHANICAL workstream exception:** For workstreams Sage classifies as MECHANICAL (low-risk, well-specified, zero-architectural-judgment tasks — e.g., string format changes, constant updates, config tweaks), dev handles the full TDD cycle including test authorship. QA is not spawned for MECHANICAL workstreams. This exception does NOT apply to ARCHITECTURAL workstreams, or any workstream containing new business logic, schema changes, external API integrations, or security-relevant changes. The MECHANICAL classification is Sage's responsibility at workstream intake — qa does not self-classify workstreams as MECHANICAL.
