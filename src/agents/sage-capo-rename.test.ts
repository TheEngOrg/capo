...
// =============================================================================
// sage-capo-rename.test.ts — RED specs for WS-RENAME-T2-FIX
//
...
//   agent_id: "sage" in src/adapters/claude-code.test.ts (attack payload under test)
...
//   "sage-like-agent" in spawn-agent.test.ts — fixture name, not persona ref
//
 ...
 *
 * agent_id=sage / agent_id: "sage" / "task-sage-bad" / "sage" non-executor —
 *   Attack-payload strings in prompt-injection test (claude-code.test.ts).
 *   The literal "sage" is the VALUE BEING REJECTED, not a persona reference.
 *
 ...
 *
 * sage-like-agent — fixture name in spawn-agent.test.ts, not persona ref.
 *
...
describe("golden: protected patterns are still present (no over-correction)", () => {
  ...
  it("attack-payload string 'agent_id: sage' still appears in claude-code.test.ts", () => {
    // GOLDEN: the prompt-injection test must continue to test rejection of
    // "sage" as an invalid agent_id. This string is the value being rejected,
    ...
    expect(content.toLowerCase()).toContain('agent_id: "sage"');
  });
});
