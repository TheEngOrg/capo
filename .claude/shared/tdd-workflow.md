...
**Owner:** teo-build orchestrator (before any agent spawns)
...
`teo-check-test-ordering` enforces this ordering mechanically at pre-commit and task-completion.
...
For complex verification checks, verification scripts MUST be written via Write tool to `/tmp/teo-verify-*.sh`, then executed with Bash tool. This prevents settings.local.json permission pattern bloat from inline bash heredocs.
...
```
# After ARCHITECTURAL Step 3 (staff-engineer review) passes:
