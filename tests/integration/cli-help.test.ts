...
// T-05 — --help exits 0 with non-empty stdout
// Runs against `bun src/index.tsx --help` in dev mode.
// Pass 2: also run against the compiled binary.
describe('CLI --help (T-05)', () => {
  it('exits 0 with non-empty stdout', () => {
    ...
    expect(result.stdout.trim().length, '--help produced empty stdout').toBeGreaterThan(0);
  });
});
