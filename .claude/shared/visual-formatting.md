...
INDIGO='\033[0;94m'     # CAPO — orchestrator (TEO-only)
...
Displayed at the start of every Capo session and every `/teo` invocation. This is the first thing the user sees.

```
╔══════════════════════════════════════════════════════════════╗
║  🧘 TheEngOrg Enterprise v2.0.0                             ║
║  📦 Project: {project-name}                                  ║
║  🔧 TEO Base: {teo-version} | Capo: Active                  ║
╚══════════════════════════════════════════════════════════════╝
```
...
**When to display:**
- At Capo session start (after intake, before pipeline)
...
|-------|----------|-------|--------|
| 🟡 [LEAD] | Leadership | GOLD `\033[0;33m` | cto, engineering-director, product-owner |
...
| 🟣 [CREATE] | Creative | PURPLE `\033[0;35m` | design, art-director, ai-artist, technical-writer, studio-director |
| ⚪ [COORD] | Coordination | GRAY `\033[0;90m` | supervisor, engineering-manager, product-manager, api-designer |
| 🔮 [CAPO] | Orchestrator | INDIGO `\033[0;94m` | capo |
| 🛡️ [COMPLIANCE] | Enforcement | RED `\033[0;31m` | compliance-officer, legal-counsel |

**TEO-only badges:** CAPO and COMPLIANCE are available in the full TEO distribution. TEO community edition does not define or use these badges.
...
🔮 [CAPO] capo: pipeline PLAN composed (2s)
...
Displayed by Capo after each pipeline step completes. Shows the full pipeline with current position.

```
🔮 [CAPO] Pipeline: {INTENT}
  ├─ ✅ Step 1: teo-assess (complete)
  ├─ ✅ Step 2: teo-spec (complete)
  ├─ 🔄 Step 3: teo-build (in progress)
  └─ ⏳ Step 4: staff-engineer (pending)
```
