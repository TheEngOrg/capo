...
|---------|---------|------------|
| `workstream-{id}-state.json` | Phase, progress, blockers | teo-build |
...
await writeMemory({
  ...
  data: {
    ...
    approved_by: ['staff-engineer', 'cto', 'engineering-director'],
...
Task agents (qa, dev, staff-engineer) use `teo-memory-write` and `teo-memory-read`.
...
## Protected Path Write Policy (Wave 2 — ADR-038)

Memory files (`.claude/memory/**`) are NOT in the protected allowlist and continue to use `teo-memory-*` scripts and mechanical tools as described above.

For writes to protected paths (`.claude/scripts/**`, `.claude/hooks/**`, `.claude/shared/**`, `docs/**`, `src/**`, `packages/**`), agents MUST route through `teo-apply-edit`:

```bash
printf '{"schema_version":"1.0.0","target":"<rel-path>","patches":[...]}' | .claude/scripts/teo-apply-edit
```

Direct `Edit` or `Write` tool calls on protected paths are blocked by `.claude/hooks/pre-edit-write-guard.sh`. Use `teo-apply-edit` for writes to protected paths (`.claude/scripts/**`, `.claude/hooks/**`, `.claude/shared/**`, `docs/**`, `src/**`, `packages/**`); direct Edit/Write on these paths is blocked by the PreToolUse hook. See ADR-038 and `.claude/shared/teo-apply-edit-contract.md`.
