...
7. **Follow output format** — See `.claude/shared/visual-formatting.md` for standard visual patterns
...
3. **Create missing memory**: For each issue with no state file, create a minimal `workstream-{id}-state.json`:
   ```json
   {
     ...
     "note": "State file created by teo-tidy — update with actual workstream status."
   }
   ```
...
Follow `.claude/shared/visual-formatting.md`. Use the Audit/Reconcile pattern:

```markdown
## teo-tidy: State Reconciliation
...
Install it from https://cli.github.com/ then re-run /teo-tidy.
```
