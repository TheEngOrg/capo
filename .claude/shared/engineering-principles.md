...
1. **Write the script to a file** using the Write tool (e.g., `/tmp/teo-verify-*.sh`)
...
# (Use Write tool to create /tmp/teo-verify-coverage.sh with verification logic)
bash /tmp/teo-verify-coverage.sh
...
Every agent spawn has a token cost. Be intentional.

### Complexity-Matched Agent Counts

| Task Type | Agent Count | Example |
|-----------|------------|---------|
| MECHANICAL | 1 agent (dev) | Bug fix, config change, simple feature |
| ARCHITECTURAL | 5-6 agents | New system design, cross-cutting refactor |

Match agent count to complexity. Do not spawn a leadership team for a typo fix.

### Capo Tiered Routing

- **Skip Capo** for MECHANICAL tasks — dev handles full TDD cycle alone
- **Spawn Capo** for ARCHITECTURAL tasks and ambiguous scope
- When in doubt: start MECHANICAL, escalate to ARCHITECTURAL if blockers emerge

### Model Tier Assignment

| Agent | Tier | Rationale |
|-------|------|-----------|
| Capo | opus | Orchestration requires deep reasoning |
| dev, qa, eng-manager | sonnet | Sufficient for implementation, review, delegation |
| supervisor, deployment-engineer, researcher | haiku | Lightweight coordination tasks |

Never upgrade an agent's model tier unless explicitly instructed. Opus costs 5x sonnet.

### Budget Awareness

- `dailyMode.maxTokensPerRun` = **50,000** (hard cap)
- MECHANICAL workstreams always use sonnet; ARCHITECTURAL may use opus for planning

### Cost Optimization Habits

1. Use MECHANICAL classification by default — only escalate when architecture is genuinely affected
2. Batch tool reads; avoid re-reading files already loaded in context
3. Prefer shallow context (single agent) over team spawns for isolated tasks
