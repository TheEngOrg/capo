...
The Process Matcher is the mechanism the Sage uses to match incoming requests against registered process flows. It sits between intent classification and pipeline composition in the Sage's intake flow:

```
Intake -> Research Gate -> Classify Intent -> Process Match -> Compose Pipeline + Flow -> Execute
```

When the Sage classifies intent, the matcher determines which registered flow (if any) should govern execution. If no flow matches, the Sage falls back to the planning-spike flow for discovery.
...
1. **Matched flow** (HIGH confidence) -- a single flow with score >= 80
   - The Sage loads this flow and uses it to govern pipeline execution
...
2. **Candidate flows** (MEDIUM confidence) -- one or more flows with scores 30-79
   - The Sage presents candidates to the user:
...
3. **No match** (LOW confidence) -- all flows scored below 30
   - The Sage falls back to the planning-spike flow
...
After the process matcher returns a result and intent classification completes (in parallel), the Sage checks whether the matched flow and classified intent are aligned.
...
When both cycles fail to align, the Sage presents a structured decision prompt to the user. The escalation includes:
...
1. **HIGH confidence match on a composed flow:**
   - The Sage uses the composed flow's `phase_order` and `composes` list to build the pipeline
...
2. The Sage re-runs the matcher with the original request context
...
## Process Matcher in the Sage's Intake Flow
...
When debug mode is active (`debug: true` in the Sage intake), the process matcher produces verbose output for every flow it evaluates. This output goes to both console (stderr) and `.claude/memory/traces/debug-log.json`.
...
Log all match results to the pipeline log (`.claude/memory/sage-pipeline-log.json`):
