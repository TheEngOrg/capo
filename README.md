...
- **Deterministic guardrails** — the gate logic, plan validation, and audit signing are deterministic code, not LLM judgment. The agent work they wrap is not — TEO's job is to put hard, verifiable guardrails around it.
- **Signed audit trail** — gate verdicts are recorded in an HMAC-signed, append-only ledger you can verify independently.
...
- It does not require an API key, a license, or a login. The agents use your existing session.
