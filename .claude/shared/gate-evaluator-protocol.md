...
config:
  file: ".claude/agents/sage/agent.md"
...
```json
{
  ...
  "override": {
    "type": "force-proceed",
    "timestamp": "ISO8601",
    "session_id": "sage-2026-03-25-001",
...
Every gate evaluation writes to the pipeline log (`.claude/memory/sage-pipeline-log.json`):
...
Human-resolution gates (`resolution: human`) are never auto-evaluated. When the evaluator encounters a gate with `resolution: human`, it returns SKIPPED with reason `human_gate` and the Sage presents the gate to the user per the escalation format in `gate-classification-protocol.md`.
...
- **Phase 2 gate evaluator**: Fires during Sage pipeline execution. The Sage reads gate definitions from the registry and evaluates them.

The two layers are complementary:
- Hooks catch issues when agents complete tasks (any agent, any task)
- The gate evaluator catches issues during orchestrated pipeline execution (Sage-managed work)
